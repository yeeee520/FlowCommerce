...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      // ========== 项目 2：超市后台管理系统 ==========
      projectTitle("2026.05 - 2026.07", "超市后台管理系统", "Spring Boot 3 + Vue 3 前后端分离", "全栈开发"),

      projTag("项目概述"),
      bullet("【AI 辅助开发】基于 Claude CLI/OpenCode 驱动开发全流程，集成 Kimi/DeepSeek 进行代码审查、接口测试与自动化部署。设计 7 张关联数据库表（用户、管理员、分类、公告、预约、资源等），基于 Spring Boot 3 + MyBatis + PageHelper 实现分页查询，5000+ 条数据翻页响应 < 50ms"),
      bullet("使用 Apache POI 封装 Excel 通用导入导出工具，支持模板下载与万级数据批量入库，单次 5000 条导入耗时 < 3s"),
      bullet("基于 java-jwt 实现 Token 签发与校验，配合拦截器完成 15 个权限点的控制，双角色（管理员/普通用户）动态菜单渲染"),
      bullet("集成 WangEditor 富文本编辑器实现内容排版；对接 ECharts 后端统计接口，展示柱状图、折线图、饼图三类可视化报表"),
      bullet("实现文件上传/下载/预览（图片与文档），结合审核状态机完成预约 -> 审核 -> 资源分配三层级联业务，端到端覆盖 5 个状态流转"),

      projTag("技术栈"),
      bullet("Java / Spring Boot 3 / MyBatis / Vue 3 / MySQL / JWT / Apache POI / ECharts / WangEditor"),
...
// 生成文件
// ============================================================
const outputPath = "C:/Users/yeeee/Desktop/求职简历/yeeee/叶泳君_简历_超市+AIagent.docx";
